...
@app.post("/api/v1/calculate/divide", response_model=DivisionResponse)
def divide(payload: DivisionRequest) -> DivisionResponse:
    if payload.operand_b == 0:
        raise HTTPException(status_code=400, detail="operand_b must not be 0")
